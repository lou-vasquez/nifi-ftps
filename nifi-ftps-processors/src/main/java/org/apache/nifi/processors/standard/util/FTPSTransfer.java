...
/**
 * Take over the complete file since the client/getClient are made private so we cannot override them...
 * What a pitty...
 */
public class FTPSTransfer implements FileTransfer {
    ...
    public static final PropertyDescriptor UTF8_ENCODING = new PropertyDescriptor.Builder()
            ...
            .build();
    public static final PropertyDescriptor ALLOW_SELFSIGNED = new PropertyDescriptor.Builder()
            .name("allow_selfsigned")
            .displayName("Allow self-signed certs")
            .description("Do we allow the remote host to identify himself with a self-signed certificate?")
            .required(true)
            .allowableValues("true", "false")
            .defaultValue("false")
            .addValidator(StandardValidators.BOOLEAN_VALIDATOR)
    ...
    public FTPSTransfer(final ProcessContext context, final ComponentLog logger) {
        this.ctx = context;
        this.logger = logger;
    }
